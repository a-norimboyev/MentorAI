...
const roleConfig = {
  ...
  teacher: {
    ...
    buttonBg: 'bg-green-600 hover:bg-green-700 disabled:bg-green-600/50',
    hasSubject: true
  },
  'self-learner': {
    title: "Mustaqil o'rganuvchi",
    icon: GraduationCap,
    iconBg: 'bg-purple-500/20',
    iconColor: 'text-purple-500',
    buttonBg: 'bg-purple-600 hover:bg-purple-700 disabled:bg-purple-600/50',
    hasSubject: false
  }
}
...
const RegisterForm = ({ role }) => {
  ...
  const validateForm = () => {
    ...
    if (config.hasSubject && !formData.subject.trim()) errors.push('Fanni tanlash majbur')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const extraData = config.hasSubject ? { subject: formData.subject } : undefined
      await register(formData.email, formData.password, formData.name, role, extraData)
        ...
        <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full ${config.buttonBg} text-white font-medium py-3 rounded-lg transition flex items-center justify-center gap-2`}
            >
              {loading ? (
                <>
